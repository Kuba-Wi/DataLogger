...
static uint8_t QSPI_DummyCyclesCfg();
...
uint8_t BSP_QSPI_Erase_Block(uint32_t BlockAddress)
{
  ...
  if (QSPI_AutoPollingMemReady(N25Q128A_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t BSP_QSPI_Erase_Chip(void)
{
  ...
  /* Configure automatic polling mode to wait for end of erase */
  if (QSPI_AutoPollingMemReady(N25Q128A_BULK_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
